...
def main():
    simple_algo(all_boards)


def simple_algo(boards):
    for board in boards:
        print('\n\n\n*****     Starting Board     *****')
        for row in board:
            print(row)
            ...
            while x <= 8:
                ...
                while y <= 8:
                    if board[y][x] == 0:  # If the current cell has not been confirmed
                        square_list = get_cells_in_this_square(x, y)

                        set_of_numbers_intersecting = set()  # Reset the list of intersecting numbers

                        i = 0
                        while i <= 8:
                            set_of_numbers_intersecting.add(board[y][i])
                            set_of_numbers_intersecting.add(board[i][x])
                        ...
                        dictionary_of_candidates[(x, y)] = possible_candidates
...
def get_cells_in_this_square(x_now, y_now):
